...
class YcmModule extends CWebModule {
	...
	private function initMenuItems() {
		foreach ($this->menuItems as $key => $item) {
			if ($this->menuItems[$key]['model'])
				$this->menuItems[$key]['url'] = array('model/list', 'name' => $this->menuItems[$key]['model']);
			if ($this->menuItems[$key]['items'])
				$this->menuItems[$key]['items'] = $this->getMenuSubitems($this->menuItems[$key]['items']);
		}
	}

	private function getMenuSubitems($items) {
		foreach ($items as $key => $item) {
			if ($items[$key]['model'])
				$items[$key]['url'] = array('model/list', 'name' => $items[$key]['model']);
			if ($items[$key]['items'])
				$items[$key]['items'] = $this->getMenuSubitems($items[$key]['items']);
		}
		return $items;
	}
